...
namespace DL
{
    public class ReviewDL : IReviewDL
    {
        ...
        {
            IQueryable<ReviewDbDto> _reviews = _db.Reviews.Include(r => r.Place).Include(r => r.User);

            if (_placeId != null)
            {
                PlaceDbDto place = await _db.Places.FirstOrDefaultAsync(p => p.Id == _placeId);
                if (place == null)
                {
            ...
            if (_userId != null)
            {
                UserDbDto user = await _db.Users.FirstOrDefaultAsync(u => u.Id == _userId);
                if (user == null)
                {
